...
@JvmOverloads constructor(
    private var printResponseHeader: Boolean = false,
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        // 1. 请求第一行
        var requestMessage =
            "$requestPrefix ${request.method()} ${request.url()} ${connection?.protocol()}\n"
        // 2. 请求头，只拼自定义的头
        ...
        var responseMessage =
            "$responsePrefix ${response.code()} ${if (response.message().isEmpty()) "" else response.message()} "
